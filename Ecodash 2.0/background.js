...
const AI_CONFIG = {
  ...
  TEMPERATURE: 0.7,
  MAX_TOKENS: 180,
  RESPONSE_STYLE: `Respond with exactly 3 bullet points formatted as:
"- [Action] (X% savings): [1-sentence explanation]"
Example: 
"- Lower video quality (50% savings): Reduces bandwidth usage."
Keep explanations concise and professional. No emojis.`
};
...
async function generateAITips() {
  try {
    ...
    const tips = await getGroqResponse(
      `Analyze these usage patterns: ${JSON.stringify(services)}`
    );
    chrome.storage.local.set({ aiTips: tips });
  } catch (error) {
    console.error("AI analysis failed:", error);
    chrome.storage.local.set({
      aiTips: "- Lower video quality (50% savings): Reduces data usage significantly.\n- Audio-only mode (75% savings): Eliminates video processing energy.\n- Close unused tabs (15% savings): Stops background network requests."
    });
  }
}
...
async function testAITips() {
  const tips = await getGroqResponse("Test connection");
  console.log(tips);
}